...
class MistralFineTuner(pl.LightningModule):
    """PyTorch Lightning Mistral Model class"""

    def __init__(self, hparams, tokenizer, model):
        """initiates a PyTorch Lightning Mistral Model"""
        super().__init__()
    ...
    def _forward(self, input_ids, attention_mask, labels=None):
        output = self.model(
            input_ids,
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        path = f"{self.outputdir}/Mistral-epoch-{self.current_epoch}-train-loss-{str(self.average_training_loss)}-val-loss-{str(self.average_validation_loss)}"
    ...
    def validation_epoch_end(self, validation_step_outputs):
        ...
        self.average_validation_loss = np.round(
            torch.mean(torch.stack(_loss)).item(), 4
        )
