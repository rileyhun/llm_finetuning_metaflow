...
def prompt_tokenize(
    prompt, completion, tokenizer, max_len, truncation=True, padding=True
):
    ...
    loss_mask[
        :, list(range(len(prompt_toks), len(prompt_toks) + len(completion_toks)))
    ] = True
    attention_mask = torch.ones(sample.shape, dtype=int)
    if padding:
        pad_zeros = torch.nn.ConstantPad1d((0, max_len - sample.shape[1]), 0)
        pad_eos = torch.nn.ConstantPad1d(
            (0, max_len - sample.shape[1]), tokenizer.pad_token_id
        )
...
Category: """
    ...
    def __init__(
        self, data_df, tokenizer, max_prompt_len=100, truncation=True, padding=True
    ):
        self.df = data_df
    ...
    def __getitem__(self, idx):
        ...
        input_ids, attention_mask, loss_mask = prompt_tokenize(
            prompt,
            completion,
            self.tokenizer,
            self.max_prompt_len,
